    ...
    , ui(new Ui::MainWindow)
{
    ...
    QMenu* Insmenu = menuBar()->addMenu(tr("Insert"));
    QAction* mInsTime = Insmenu->addAction(tr("Insert time"));

    connect(mInsTime, SIGNAL(triggered(bool)), this, SLOT(InsertTime()));
...
{
    QDateTime insDT = QDateTime::currentDateTime();
    QString insert = " (" + QString::number(insDT.date().day()) + '.' + QString::number(insDT.date().month()) + '.' + QString::number(insDT.date().year()) + ") " + insDT.time().toString();
    ui->Editor->textCursor().insertText(insert);
}
